...
async function start(client: PrismaClient) {
  console.log(
    `Fetching pools from ${PROTOCOL} ${VERSION}, chains: ${DFYN_V2_SUPPORTED_CHAINS.map(
      (chainId) => chainName[chainId]
    ).join(', ')}`
  )

  let totalPairCount = 0
  for (const chainId of DFYN_V2_SUPPORTED_CHAINS) {
    ...
    const sdk = getBuiltGraphSDK({ chainId, host: GRAPH_HOST[chainId], name: DFYN_V2_SUBGRAPH_NAME[chainId] })
    if (!DFYN_V2_SUBGRAPH_NAME[chainId]) {
      console.log(`Subgraph not found: ${chainId} ${DFYN_V2_SUBGRAPH_NAME[chainId]}, Skipping`)
      continue
    }
    console.log(`Loading data from chain: ${chainName[chainId]}(${chainId}), ${DFYN_V2_SUBGRAPH_NAME[chainId]}`)
    let pairCount = 0
    let cursor = ''
    ...
    console.log(
      `Finished loading pairs from ${GRAPH_HOST[chainId]}/${DFYN_V2_SUBGRAPH_NAME[chainId]}, ${pairCount} pairs`
    )
  }
